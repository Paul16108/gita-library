...
function performSearch(query) {
  if (!query || query.trim().length < 2) {
    // Show all chapters when search is empty
    showChaptersView();
    return;
  }
    ...
    chapter.verses.forEach((verse, verseIndex) => {
      if (verse.content.toLowerCase().includes(searchTerm)) {
        results.push({
          ...
          preview: getSearchPreview(verse.content, searchTerm)
        });
      }
    });
  });
...
function getSearchPreview(content, searchTerm) {
...
function displaySearchResults(results, searchTerm) {
  ...
  if (!searchResults) {
    ...
    elements.searchSection.appendChild(searchResults);
  }
  ...
  results.forEach(result => {
    ...
    resultItem.innerHTML = `
      <div class="result-header">
        <span class="result-verse">Стих ${result.verseNumber}</span>
      </div>
      <div class="result-preview">${result.preview}</div>
    `;
    
    resultItem.addEventListener('click', () => {
      showVerseView(result.chapter, result.verseIndex);
